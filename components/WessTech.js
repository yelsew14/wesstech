...
const NEWS_SOURCES = [
  ...
  { name:'ZDNet Security',     url:'https://www.zdnet.com/topic/security/',           flag:'🔐' },
...
export default function WessTech({ articles = [], fetchedAt, feedError }) {
  ...
  const lastUpdated = fetchedAt
    ? new Date(fetchedAt).toLocaleTimeString('en-NZ', { hour:'2-digit', minute:'2-digit' })
    : ''
  ...
  return (
    <div style={{ background:'#060a14', minHeight:'100vh', color:'#e2e8f0', fontFamily:"'Space Grotesk',sans-serif" }}>
      ...
      {/* ── NAV ── */}
      <nav style={{ background:'#030711', borderBottom:'1px solid #1e2d47', padding:'0 24px', position:'sticky', top:0, zIndex:100 }}>
        <div style={{ maxWidth:1200, margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between', height:56, gap:16 }}>
          ...
          <div style={{ flex:1, maxWidth:380, position:'relative' }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)}
              ...
              className="sg"
              style={{ width:'100%', background:'#0d1424', border:'1px solid #1e2d47', borderRadius:8, padding:'7px 12px 7px 36px', fontSize:13, color:'#e2e8f0' }} />
          </div>

          <div style={{ display:'flex', gap:8, alignItems:'center', flexShrink:0 }}>
            {/* Live indicator */}
            <div style={{ display:'flex', alignItems:'center', gap:6, background:'#0d1424', border:'1px solid #1e2d47', borderRadius:20, padding:'4px 12px' }}>
              <div style={{ width:6, height:6, background:'#16a34a', borderRadius:'50%', animation:'pulse 1.5s ease-in-out infinite' }} />
              <span className="sg" style={{ color:'#6b7280', fontSize:11 }}>Live · {lastUpdated}</span>
            </div>
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 24px 80px' }}>
        ...
        {/* ── HERO ── */}
        <div style={{ padding:'44px 0 32px', borderBottom:'1px solid #1e2d47', marginBottom:32 }}>
          <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
            <div style={{ width:8, height:8, background:'#16a34a', borderRadius:'50%', animation:'pulse 1.5s ease-in-out infinite' }} />
            <span className="sg" style={{ color:'#374151', fontSize:12, letterSpacing:'.1em', textTransform:'uppercase' }}>
              Live RSS Feed · {articles.length} articles · Updates every hour
            </span>
          </div>
          <h1 className="sy" style={{ fontSize:'clamp(28px,5vw,50px)', fontWeight:800, lineHeight:1.05, marginBottom:14, letterSpacing:'-.03em' }}>
            AI & Tech News<br />
          ...
          <p className="sg" style={{ color:'#6b7280', fontSize:15, maxWidth:520, lineHeight:1.7 }}>
            Live news from NZ Herald, RNZ, TechCrunch, The Verge, Wired, CERT NZ and more — pulled directly from RSS feeds and refreshed every hour automatically.
          </p>

          {/* Feed error banner */}
          {feedError && (
            <div style={{ background:'#7f1d1d22', border:'1px solid #dc262644', borderRadius:10, padding:'10px 16px', marginTop:16, display:'inline-block' }}>
              <span className="sg" style={{ color:'#fca5a5', fontSize:13 }}>⚠️ Some feeds temporarily unavailable — showing cached results.</span>
            </div>
          )}
        </div>
          ...
          <div style={{ marginBottom:32 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
              <div style={{ background:'#dc2626', borderRadius:4, padding:'2px 8px' }}>
                <span className="sg" style={{ fontSize:10, fontWeight:700, color:'#fff', letterSpacing:'.1em' }}>BREAKING</span>
              </div>
              <span className="sg" style={{ color:'#6b7280', fontSize:12 }}>Latest stories from the last 3 hours</span>
            </div>
              ...
              {hot.map(n => {
                ...
                return (
                  <a key={n.id} href={n.url} target="_blank" rel="noopener noreferrer">
                    <div className="card fi" style={{ padding:'14px 16px', borderLeft:`3px solid ${cc.dot}`, cursor:'pointer' }}>
                      ...
                      <p className="sg" style={{ fontSize:13, fontWeight:600, color:'#e2e8f0', lineHeight:1.5 }}>{n.title}</p>
                      <div className="sg" style={{ fontSize:11, color:'#374151', marginTop:6 }}>{n.source} ↗</div>
                    </div>
                  </a>
                )
              })}
        ...
        <div style={{ display:'flex', gap:8, overflowX:'auto', paddingBottom:8, marginBottom:24 }}>
          {CATEGORIES.map(c => {
            const count = articles.filter(a => c.id === 'all' || a.cat === c.id).length
            return (
              <button key={c.id} className="nb sg ch"
                onClick={() => { setActivecat(c.id); setShowSaved(false) }}
                style={{ padding:'7px 14px', borderRadius:20, fontSize:12.5, fontWeight:600, whiteSpace:'nowrap',
                ...
                {c.icon} {c.label}
                <span style={{ marginLeft:6, background: activecat===c.id && !showSaved ? '#ffffff22' : '#1e2d47', borderRadius:10, padding:'1px 6px', fontSize:10 }}>
                  {count}
                </span>
              </button>
            )
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign:'center', padding:'60px 0', color:'#374151' }}>
            ...
            <p className="sg" style={{ fontSize:15 }}>
              {showSaved ? 'No saved articles yet — click 🗄️ on any article to save it' : 'No results found'}
            </p>
          </div>
        ) : (
          <div className="grid fi">
            {filtered.map(n => {
              ...
              return (
                <a key={n.id} href={n.url} target="_blank" rel="noopener noreferrer">
                  <div className="card" style={{ padding:'18px 18px', height:'100%', display:'flex', flexDirection:'column', gap:10 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                        {n.hot && <span style={{ width:7, height:7, background:'#dc2626', borderRadius:'50%', animation:'pulse 1.5s ease-in-out infinite', display:'inline-block' }} />}
                        <button className="nb" onClick={e => toggleSave(n.id, e)}
                          style={{ fontSize:15, color: isSaved ? '#f59e0b' : '#374151', lineHeight:1 }}>
                          {isSaved ? '🔖' : '🗄️'}
                        </button>
                      </div>
                    </div>
                    <h3 className="sg" style={{ fontSize:14, fontWeight:700, color:'#f1f5f9', lineHeight:1.5, flex:1 }}>{n.title}</h3>
                    <p className="sg" style={{ fontSize:12.5, color:'#6b7280', lineHeight:1.6 }}>{n.summary}</p>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', paddingTop:10, borderTop:'1px solid #1e2d47' }}>
        ...
        {/* ── HOW IT WORKS ── */}
        {!showSaved && search === '' && (
          <div style={{ marginTop:48, background:'#0d1424', border:'1px solid #1e2d47', borderRadius:14, padding:'20px 22px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
              <div style={{ width:8, height:8, background:'#16a34a', borderRadius:'50%', animation:'pulse 1.5s ease-in-out infinite' }} />
              <span className="sg" style={{ color:'#6b7280', fontSize:12, fontWeight:600 }}>How live feeds work</span>
            </div>
            <p className="sg" style={{ color:'#4b5563', fontSize:12.5, lineHeight:1.7 }}>
              WessTech pulls directly from RSS feeds published by {NEWS_SOURCES.length} news sources. Vercel's server fetches and caches the feeds every hour — so you always see real, current news without page reloads. Articles link directly to the original publisher.
            </p>
          </div>
        )}

        {/* ── SOURCES ── */}
        <div style={{ marginTop:40, paddingTop:28, borderTop:'1px solid #1e2d47' }}>
          <h2 className="sy" style={{ fontSize:18, fontWeight:800, color:'#f1f5f9', marginBottom:16, letterSpacing:'-.02em' }}>Live Sources</h2>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(170px,1fr))', gap:8 }}>
            {NEWS_SOURCES.map((s,i) => (
              <a key={i} href={s.url} target="_blank" rel="noopener noreferrer">
                <div className="card ch" style={{ padding:'11px 14px', display:'flex', alignItems:'center', gap:10 }}>
                  <span style={{ fontSize:16 }}>{s.flag}</span>
                  <span className="sg" style={{ fontSize:12.5, color:'#94a3b8' }}>{s.name}</span>
                </div>
              </a>
            ))}
          </div>
        ...
        {/* ── ABOUT + LINKS ── */}
        <div style={{ marginTop:32, background:'#0d1424', border:'1px solid #1e2d47', borderRadius:16, padding:'24px 24px' }}>
          <div style={{ display:'flex', gap:16, flexWrap:'wrap', alignItems:'flex-start' }}>
            <div style={{ flex:1, minWidth:240 }}>
              <h3 className="sy" style={{ fontSize:17, fontWeight:800, color:'#f1f5f9', marginBottom:8 }}>About WessTech</h3>
              <p className="sg" style={{ color:'#6b7280', fontSize:13, lineHeight:1.7 }}>
                WessTech aggregates live AI and technology news from New Zealand and around the world via RSS feeds. Updated every hour automatically. Built by <a href="https://wesstech.xyz" style={{ color:'#2563eb' }}>wesstech.xyz</a>.
              </p>
            </div>
            <div style={{ display:'flex', flexDirection:'column', gap:8, minWidth:180 }}>
              {[
                { label:'About WessTech', url:'/about', icon:'ℹ️' },
                { label:'PathwayNZ — Filipino NZ Guide', url:'https://pathway.wesstech.xyz', icon:'🥝' },
              ].map((l,i) => (
                <a key={i} href={l.url} target={l.url.startsWith('http') ? '_blank' : '_self'}
                  ...
                  <div style={{ background:'#060a14', border:'1px solid #1e2d47', borderRadius:10, padding:'10px 14px', display:'flex', gap:10, alignItems:'center' }}>
                    ...
                    <span className="sg" style={{ fontSize:12.5, color:'#e2e8f0' }}>{l.label}</span>
                  </div>
                </a>
              ))}
            </div>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', flexWrap:'wrap', gap:20, marginBottom:20 }}>
            <div>
              ...
              <p className="sg" style={{ color:'#6b7280', fontSize:12.5, maxWidth:300, lineHeight:1.7 }}>
                Live AI and technology news for New Zealand and the world — pulled from RSS feeds, refreshed every hour. Also home to PathwayNZ, a free guide for Filipino migrants in New Zealand.
              </p>
            </div>
